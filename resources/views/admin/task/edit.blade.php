    ...
    <div class="container">
        ...
        <div class="row">
            <div class="col-md-12">
                ...
                <a href="{{route('tasks.show', $task->id)}}">Текущая задача</a>
            </div>
                ...
                <form action="{{route('tasks.update', $task->id)}}" method="post" autocomplete="off">
                    ...
                    <div class="form-group">
                        ...
                        <div class='input-group date' id='datetimepicker1'>
                            <input name="time" type='text' class="form-control input-group-addon" value="{{old('time') ?? $task->time}}" />
                        </div>
                    </div>
                                ...
                                @foreach($statuses as $status)
                                    <option value="{{$status->id}}"
                                            {{
                                                ...
                                                ||
                                                (!old('status_id') && $task->status && ($task->status->id == $status->id))
                                                    ? 'selected'
                                                    : ''
                                            }}
                                    >
                    ...
                    <div class="form-group">
                        <textarea name="description" id="" cols="30" rows="10">{{old('description') ?? $task->description}}</textarea>
                    </div>
